...
class Tareth:
    ...
    def issac_rel(self, inventory):
        self.response = Fore.RED + "[TARETH]: We were good friends since college. We ended up going to the same " \
    ...
    def william_rel(self, inventory):
        self.response = Fore.RED + "[TARETH]: The kid? I never liked him much. He's okay, but nothing special - too coddled by his mother and father to truly understand what it takes to succeed.\n   1 - Agree\n   2 - Disagree\n"
        slow_print(self.response)
        self.reply = input("> ")
        while self.reply not in ["1", "2"]:
            print("Invalid, try again")
    ...
    def mia_rel(self, inventory):
        self.response = Fore.RED + "[TARETH]: Her? Never really had much trouble with her to be honest. She was always the quiet one out of his kids. A talented artist, for sure, but she had little interest in her father's business\n   1 - Ask if she ever argued with her dad\n   2 - Say she sounds like a good kid\n"
        slow_print(self.response)
        self.reply = input("> ")
        while self.reply not in ["1", "2"]:
            print("Invalid, try again")
    ...
    def janet_rel(self, inventory):
        self.response = Fore.RED + "[TARETH]: Oh... her. I never really liked her. From the moment we met in Issac's office, we never got along.\n   1 - Ask why\n   2 - Ask if he thinks she did it\n   3 - Say she can't be that bad\n"
        slow_print(self.response)
        self.reply = InputValidation.check2()
        if self.reply == "1":
            self.response = Fore.YELLOW + "[YOU]: Why did you not like her?\n\n" + Fore.RED + "[TARETH]: She was always trying to boss him around, even at work. It was like it was her company, not ours.\n  1 - Ask why he came\n   2 - Say nothing\n"
            self.reply = InputValidation.check3()
            if self.reply == "1":
                self.response = Fore.YELLOW + "[YOU]: If you have such a problem with her then why did you come tonight?\n\n" + Fore.RED + "[TARETH]: Issac's my friend, and as much as I don't like her, I wanted to be there for him, like the old days... I...I guess I've lost all that now.\n"
            else:
                self.response = "\n"
        elif self.reply == "2":
            self.response =  Fore.YELLOW +"[YOU]: Do you think she did it?\n\n" + Fore.RED + "[TARETH]: As bad as she may be, I don't think that she had the guts to do something like this. At least I hope not."
        else:
            self.response = Fore.YELLOW + "[YOU]: He's not that bad.\n\n" + Fore.RED + "[TARETH]: Hmm, thought you would've seen it like me. Nevermind.\n"
        slow_print(self.response)
        self.conversation_picker(inventory)
...
"""
            ...
            response = Fore.YELLOW + "[YOU]: I have to say. If this meant you'd get in with your biggest competitors, then I would have signed it." + Fore.RED + """
